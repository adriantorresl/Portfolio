...
main_dir <- '/Users/adriantorres/Documents/GitHub/Portfolio/projects/Bivariate Visualization'
setwd(main_dir)
...
dem <- elevatr::get_elev_raster(
  locations = nuevo_leon, z = 8,
  clip = "locations"
) |> terra::rast() |>
  terra::crop(nuevo_leon, mask = TRUE)
...
breaks <- biscale::bi_class(
  ...
  y = precipitation, style = "quantile",
  dim = 3
)
...
map <- ggplot(breaks) +
  ...
  labs(
    title = "Nuevo León: Temperature and Precipitation",
    subtitle = "Average temperature and precipitation (1981-2010)",
    caption = "Source: CHELSA | Author: Adrian Torres",
    x = "", y = ""
  ) +
